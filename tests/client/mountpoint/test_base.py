...
async def test_runner_not_available(monkeypatch, alice_user_fs, event_bus):
    ...
    monkeypatch.setattr("guardata.client.mountpoint.manager.import_function", _import)
...
def test_unhandled_crash_in_fs_operation(caplog, mountpoint_service, monkeypatch):
    ...
    monkeypatch.setattr(
        "guardata.client.mountpoint.thread_fs_access.ThreadFSAccess.entry_info", _entry_info_crash
    )
    ...
    if os.name == "nt":
        caplog.assert_occured(
            "[exception] Unhandled exception in winfsp mountpoint [guardata.client.mountpoint.winfsp_operations]"
        )

    else:
        caplog.assert_occured(
            "[exception] Unhandled exception in fuse mountpoint [guardata.client.mountpoint.fuse_operations]"
        )
