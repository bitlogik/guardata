...
async def wait_for_entries_synced(client, entries_pathes):
    ...
    client.signal_ns.connect(ClientEvent.FS_ENTRY_SYNCED, _on_entry_synced)
    try:
        yield event
        await event.wait()

    finally:
        client.signal_ns.disconnect(ClientEvent.FS_ENTRY_SYNCED, _on_entry_synced)


# @pytest.mark.trio
# async def test_online_sync(mock_clock, running_backend, alice_client, alice2_client2):
#     await alice_client.event_bus.spy.wait_for_backend_connection_ready()
#     await alice2_client2.event_bus.spy.wait_for_backend_connection_ready()

#     with alice_client.event_bus.listen() as events, alice2_client2.event_bus.listen() as events2:
#         await events.wait("fs.sync_monitor.idle")
#         await events2.wait("fs.sync_monitor.idle")

#         with freeze_time("2000-01-02"):
#             await alice_client.user_fs.file_create("/foo.txt")

#         with freeze_time("2000-01-03"):
#             await alice_client.user_fs.file_write("/foo.txt", b"hello world !")
...
async def test_online_sync2(autojump_clock, running_backend, client_factory, alice, alice2):
    # Given the clients are initialized while the backend is online, we are
    # guaranteed they are connected
    async with client_factory() as alice_client, client_factory() as alice2_client2:
        ...
        async with wait_for_entries_synced(alice2_client2, ["/"]), wait_for_entries_synced(
            alice_client, ("/", "/foo.txt")
        ):
...
async def test_sync_then_fast_forward_on_start(
    ...
    async with client_factory() as alice_client, client_factory() as alice2_client2:
        ...
        await alice2_client2.logout()

        with freeze_time("2000-01-04"):
            await alice_client.user_fs.file_write("/foo.txt", b"v2")
            await alice_client.user_fs.folder_create("/bar")

        async with wait_for_entries_synced(alice_client, ["/", "/bar", "/foo.txt"]):
            await alice_client.user_fs.sync()
...
async def test_fast_forward_on_offline_during_sync(
    autojump_clock, server_factory, backend, client_factory, alice, alice2
):
        ...
        async with client_factory(
            ...
            async with wait_for_entries_synced(alice_client, ("/", "/foo.txt")):
                ...
                with offline(server1.addr):
                    ...
                    with freeze_time("2000-01-04"):
                        await alice2_client2.user_fs.file_write("/foo.txt", b"v2")
                        await alice2_client2.user_fs.folder_create("/bar")

                    async with wait_for_entries_synced(alice2_client2, ("/", "/bar", "/foo.txt")):
