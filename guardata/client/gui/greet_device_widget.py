...
class Greeter:
    ...
    async def run(self, client, token):
        try:
            ...
            try:
                in_progress_ctx = await client.start_greeting_device(token=token)
            ...
            try:
                ...
                await in_progress_ctx.do_create_new_device(
                    author=client.device, device_label=in_progress_ctx.requested_device_label
                )
                await self.job_mc_send.send((True, None))
...
async def _do_send_email(client):
    try:
        return await client.new_device_invitation(send_email=True)
    except BackendNotAvailable as exc:
...
class GreetDeviceWidget(QWidget, Ui_GreetDeviceWidget):
    ...
    def __init__(self, client, jobs_ctx, invite_addr):
        super().__init__()
        self.setupUi(self)
        self.client = client
        self.jobs_ctx = jobs_ctx
    ...
    def _goto_page1(self):
        ...
        page = GreetDeviceInstructionsWidget(
            self.jobs_ctx, self.greeter, self.invite_addr, client=self.client
        )
