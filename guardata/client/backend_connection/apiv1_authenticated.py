...
from guardata.api.protocol import DeviceID, APIEvent, APIV1_AUTHENTICATED_CMDS
from guardata.client.types import BackendOrganizationAddr
from guardata.client.backend_connection import cmds
from guardata.client.backend_connection.transport import apiv1_connect, TransportPool
...
from guardata.client.backend_connection.authenticated import BackendConnStatus
        ...
        async with self._acquire_transport(ignore_status=True, force_fresh=True) as transport:
            ...
            try:
                async with trio.open_service_nursery() as monitors_nursery:
                    ...
                    self.event_bus.send(
                        ClientEvent.BACKEND_CONNECTION_CHANGED, status=self._status, status_exc=None
                    )
